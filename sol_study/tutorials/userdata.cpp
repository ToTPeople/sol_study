...
struct Doge {
	...
	~Doge() {
		std::cout << "Dog at " << this << " tailwag " << tailwag << " is being destroyed..." << std::endl;
	}
};
...
int userdata() {
	...
	lua["f"] = [](Doge& dog) {
		std::cout << "dog wags ref its tail " << dog.tailwag << " times! " << &dog << std::endl;
	};
	...
	lua["handling_f"] = [](Doge* dog) {
		...
		std::cout << "dog wags point its tail " << dog->tailwag << " times!" << dog << std::endl;
	};
